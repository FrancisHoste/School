...
/**
 * 
 * Provides a board to play four in a row
 * add checkers in turn with addChecker(player, atColumn)
 *
 */
public class FourInARow {
	...
	/**
	 * the current player adds a checker to the column of his choice, then the current player is switched
	 * @param column between 0 and 6
	 */
	public void addChecker(int column) {
		if(column < 0 || column > 6) throw new IllegalArgumentException("Invalid board position");
	...
	private int nextEmptySpot(int row, int column) {
		...
		else return nextEmptySpot(row+1, column);
	}
